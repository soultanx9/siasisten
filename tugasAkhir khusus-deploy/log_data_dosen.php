...
$dbconn = pg_connect("host=dbpg.cs.ui.ac.id port=5432 dbname=b02 user=b02 password=Xq9K3P") or die('connection failed');
 ...
 $result = pg_query($dbconn, "select k.kategori, to_char(l.tanggal,'dd-mm-yyyy') as tanggal, to_char(l.jam_mulai, 'HH24:MI') as jam_mulai,to_char(l.jam_selesai, 'HH24:MI') as jam_selesai, l.deskripsi_kerja, s.status, EXTRACT(HOUR FROM (jam_selesai - jam_mulai)) as Durasi, l.id
								 ...
								 where l.npm='$npm' and l.id_kat_log = k.id and l.id_st_log = s.id and l.idlamaran='$id' and l.idlamaran=la.idlamaran and la.idlowongan=lo.idlowongan and lo.idkelasmk=mk.idkelasmk and mk.semester='$term' order by l.id;");
